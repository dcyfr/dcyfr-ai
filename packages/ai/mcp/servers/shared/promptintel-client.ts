...
export class PromptIntelClient {
  ...
  private async handleErrorResponse(response: Response): Promise<never> {
    ...
    try {
      if (contentType?.includes('application/json')) {
        errorData = (await response.json()) as Partial<PromptIntelErrorResponse>;
      } else {
  ...
  async getPrompts(params?: PromptIntelSearchParams): Promise<PromptIntelIoPC[]> {
    ...
    const data = (await response.json()) as PromptIntelIoPC | PromptIntelIoPC[];
  ...
  async getTaxonomy(params?: PromptIntelSearchParams): Promise<PromptIntelTaxonomy[]> {
    ...
    const data = (await response.json()) as PromptIntelTaxonomy | PromptIntelTaxonomy[];
    return Array.isArray(data) ? data : [data];
  }
  ...
  async getAgentReports(
    ...
    const data = (await response.json()) as PromptIntelAgentReport | PromptIntelAgentReport[];
    return Array.isArray(data) ? data : [data];
  }
  ...
  async healthCheck(): Promise<PromptIntelHealthResponse> {
    ...
    return (await response.json()) as PromptIntelHealthResponse;
  }
}
